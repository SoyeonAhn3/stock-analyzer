...
**사전조건**: `agents/orchestrator.py`의 `run_analysis` 함수 구현 완료, `agent_overrides`로 mock 주입 가능
**실행 단계**:
1. `run_analysis(quick_look_data, agent_overrides={"news": mock, "data": mock, "macro": mock})`를 호출
2. cross_validation, analyst_agent의 `call_claude`를 mock으로 패치
3. 반환된 `analysis_state` 확인
**입력값**: NVDA quick_look_data + 3개 Agent mock 결과
...
**실행 단계**:
1. `data_agent.run("NVDA", quick_look_data)` 호출 시 `call_claude`만 mock 처리
...
2. `call_claude`를 mock하여 `{"verdict": "BUY", "confidence": "high", ...}` 반환
3. 결과의 `verdict` 값 확인
...
2. `call_claude("system", "user")` 호출 (Anthropic 클라이언트 mock 처리)
...
**실행 단계**:
1. `api_client.get_news` → `None`, `api_client.get_analyst` → `None`으로 mock
2. `call_claude`는 정상 응답 mock
...
**실행 단계**:
1. `api_client.get_macro` → `None`으로 mock
2. `call_claude`는 정상 응답 mock
...
2. macro_agent.run을 `RuntimeError`로 mock
3. cross_validation, analyst의 `call_claude`를 mock (confidence: "high" 반환)
4. `run_analysis` 호출
...
2. data, macro를 `RuntimeError`로 mock
3. analyst의 `call_claude`가 confidence: "high" 반환하도록 mock
4. `run_analysis` 호출
...
**실행 단계**:
1. `usage_tracker.is_exhausted`를 `True`로 mock
2. `call_claude("system", "user")` 호출
**입력값**: 한도 초과 상태
...
**실행 단계**:
1. news_agent.run, data_agent.run, macro_agent.run 모두 `RuntimeError` 발생하도록 mock
...
3. macro_agent.run이 10초 sleep하도록 mock
...
**실행 단계**:
1. `cross_validation.call_claude`가 실패하도록 mock (parsed: False, raw_output: "", error: "API 에러")
...
- [ ] Graceful Degradation이 설계대로 동작함 (3/3, 2/3, 1/3, 0/3)
- [ ] 일일 한도 초과 시 AI 호출이 차단됨
- [ ] 교차검증/Analyst 실패 시에도 파이프라인이 중단되지 않음
