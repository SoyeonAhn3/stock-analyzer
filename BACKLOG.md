...
| V2-2 | 결제 시스템 — 선불 크레딧 충전/판매 | 상 | 미착수 | Phase 14(지갑 구조) 선행 필요 |
...
## V2-2. 결제 시스템 — 선불 크레딧 충전/판매

> **Phase 14에서 "지갑(크레딧) 구조"를 먼저 구축**하고, 실제 결제(PG) 연동은 본 V2 항목에서 진행한다.
> Phase 14는 무료 크레딧만 지갑에 충전하며, 결제 로직은 동일 지갑에 "충전" 동작만 얹는 형태로 설계한다.

### 배경 / 비즈니스 모델

- **과금 단위**: AI 분석 버튼 1회 호출당 크레딧 1 차감 (포트폴리오 분석 / 개별주 분석 / 다중 비교 분석 등 모든 AI 분석 공통)
- **판매 방식**: 선불 크레딧 묶음 판매 (예시 가격, 확정 아님)
  - 1회 = 1,000원 / 10회 = 7,000원 (수량 할인)
- **캐시 히트는 무차감** — 새 AI 파이프라인 실행만 차감 (Phase 14 게이트와 동일 원칙)
- **무료 체험 크레딧과 유료 크레딧은 동일 지갑** — 무료분 우선 소진 후 유료분 차감

### 선결 사항 — 출시 전 반드시

| 항목 | 이유 |
|------|------|
| **회당 실제 API 원가 실측** | 분석 1회 = Claude 5회 호출(News·Data·Macro·CrossVal·Analyst). 대략 회당 200~400원 추정이나 **실측 필수**. 가격(1,000원) 대비 마진이 2~3배 수준이라 토큰 증가 시 마진 잠식 위험 |
| **무거운 분석 가중 차감 검토** | 포트폴리오 분석은 보유 종목 수만큼 토큰을 더 소비 → 균일 1크레딧이면 큰 포트폴리오에서 원가 > 가격 가능. 균일가 유지 vs 2크레딧 차등 결정 필요 |

### 핵심 설계 — 금융급 정확성 (단순 카운터 금지)

| 요소 | 내용 |
|------|------|
| **거래 원장(ledger)** | `usage_count` 단일 컬럼이 아니라 충전/차감/환불 거래 로그. "차감됐는데 결과 없음" 분쟁 대비 감사 추적 |
| **예약→확정 패턴 (Hold)** | ① 원자적 1크레딧 보류(hold) → ② 분석 실행 → ③-a 성공: 확정(commit) / ③-b 실패: 해제(release·환불). "차감 후 실패로 결과 못 받음"이 구조적으로 불가능. 동시성 보호 유지(`available = 잔액 - 보류`) |
| **멱등성(idempotency) 키** | 네트워크 재시도로 동일 요청이 2번 차감되는 것 방지. 요청별 고유 키로 중복 차감 차단 |
| **환불 필수** | 하드 실패(전 에이전트 실패·예외·타임아웃) 시 보류분 환불. 부분 성공(1~2 에이전트 실패)은 결과 제공되므로 차감 유지 |

### 글로벌 일일 한도 처리

- 기존 전역 **일일 100회 하드캡**(`utils/usage_tracker.py`)은 **유료 고객을 막으면 안 되므로 제거**
- 대신 **고임계 서킷브레이커**로 교체 — 정상 사용은 막지 않고 런어웨이 버그/어뷰징 폭주만 차단 (예: 계정당 시간당 60회 등)

### PG 연동 (본 V2 작업의 실제 범위)

| 항목 | 내용 |
|------|------|
| PG사 | 토스페이먼츠 / PortOne(아임포트) / 카카오페이 등 국내 PG 후보 검토 |
| 결제 플로우 | 크레딧 묶음 선택 → 결제 → **웹훅으로 결제 성공 확인 → 지갑에 크레딧 충전** |
| 보안 | 결제 검증은 반드시 서버 웹훅 기준 (클라이언트 신뢰 금지), 충전도 멱등 처리 |

### 선행 조건
- **Phase 14 완료** — 지갑/원장/예약-확정 인프라 구축 (무료 크레딧으로 검증)
- V2-1 외부 DB 마이그레이션 권장 (크레딧 잔액 유실 방지 — 돈이 걸린 데이터)

### 참고
- 무료 체험/지갑 기반: `services/trial_service.py` (Phase 14 신규)
- 기존 일일 한도: `utils/usage_tracker.py`
- 분석 게이트 삽입 지점: `backend/routers/analysis.py`
