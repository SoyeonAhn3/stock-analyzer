...
async def analyze(
    ...
    무료체험 게이트 (Phase 14):
        ...
        - X-Device-Id 있으면: 예약(reserve) → 성공 시 확정(commit) / 실패 시 환불(release)
        - X-Device-Id 없으면: 게이트 건너뜀 (하위 호환, 기존 동작 유지)
    ...
    # ── 무료체험 게이트: 예약 ──
    gated = bool(x_device_id)
    ref_id = x_request_id or f"auto-{uuid4()}"
    if gated:
        reserved = reserve_credit(x_device_id, ref_id)
        if not reserved["ok"]:
            raise HTTPException(
                status_code=429,
    ...
    try:
        ...
        if gated:
            release_credit(x_device_id, ref_id)
        raise
    ...
    if result.get("analyst") is None:
        if gated:
            release_credit(x_device_id, ref_id)
            result["wallet"] = get_status(x_device_id)
        return result

    # ── 성공: 캐시 저장 + 확정(차감) ──
    save_analysis(ticker, result)
    if gated:
        commit_credit(x_device_id, ref_id)
        result["wallet"] = get_status(x_device_id)
